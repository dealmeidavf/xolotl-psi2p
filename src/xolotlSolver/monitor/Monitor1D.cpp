...
// Don't do anything if it is not on the stride
  if ( (int) (time / hdf5Stride1D) == hdf5Previous1D ) PetscFunctionReturn(0);
...
// as a function of helium size
  ...
  int maxSize = 1001;  // vfda: why this is hard coded ?
// Initialize
...
// The master process writes computes the cumulative value and writes in the file
   if (procId == 0) 
   {
    outputFile << x - grid[surfacePos] << " " << heLocalConc << " " << vLocalConc 
               << " " << iLocalConc << std::endl;
   }
  }
...
// If the middle is on this MPI process
   if ( ix >= xs && ix < xs + xm ) 
   {
// Get the pointer to the beginning of the solution data for this grid point
...
// Get the data provider and give it the points
    auto thePoints = std::make_shared<std::vector<xolotlViz::Point> >( myPoints[i] );
...
// Get the maximum stable HeV cluster
  ...
  maxCluster = network->getCompound( heVType, { maxHeSize, maxVClusterSize, 0 } );
  if (!maxCluster) 
  {
...
// Get the concentration of the maximum HeV cluster
   ...
   if (maxClusterConc > 1.0e-16) maxClusterTooBig = true;
  }
...
// Compute the radius of the bubble from the number of helium
   ...
   double radius = (sqrt(3.0) / 4.0) * xolotlCore::tungstenLatticeConstant
                   + pow( (3.0 * pow(xolotlCore::tungstenLatticeConstant, 3.0) * nV) 
                          / (8.0 * xolotlCore::pi), (1.0 / 3.0) 
                        ) 
                   - pow( (3.0 * pow(xolotlCore::tungstenLatticeConstant, 3.0)) 
                          / (8.0 * xolotlCore::pi), (1.0 / 3.0)
                        );
...
// Check the option -helium_retention
  ierr = PetscOptionsHasName(NULL, NULL, "-xenon_retention", &flagXeRetention);
  checkPetscError(ierr, "setupPetsc1DMonitor: PetscOptionsHasName (-xenon_retention) failed.");
...
// Find the stride to know how often the HDF5 file has to be written
   PetscBool flag;
   ierr = PetscOptionsGetReal(NULL, NULL, "-start_stop", &hdf5Stride1D, &flag);
   checkPetscError(ierr, "setupPetsc1DMonitor: PetscOptionsGetInt (-start_stop) failed.");
   if (!flag) hdf5Stride1D = 1.0;
...
// monitorMovingSurface1D will be called at each timestep
   ...
   checkPetscError(ierr, "setupPetsc1DMonitor: TSMonitorSet (monitorMovingSurface1D) failed.");
...
// monitorBursting1D will be called at each timestep
   ...
   checkPetscError(ierr, "setupPetsc1DMonitor: TSMonitorSet (monitorBursting1D) failed.");
   std::srand(time(NULL));
  }
...
// Create a ScatterPlot
    scatterPlot1D = vizHandlerRegistry->getPlot("scatterPlot1D", xolotlViz::PlotType::SCATTER);
...
// Only the master process will create the plot
   ...
   {
// Create a ScatterPlot
    seriesPlot1D = vizHandlerRegistry->getPlot("seriesPlot1D", xolotlViz::PlotType::SERIES);
...
// Create the data provider
     auto dataProvider = std::make_shared<xolotlViz::CvsXDataProvider>( dataProviderName.str());
...
// Create a SurfacePlot
   surfacePlot1D = vizHandlerRegistry->getPlot("surfacePlot1D", xolotlViz::PlotType::SURFACE);
...
// computeFluence will be called at each timestep
   ...
   checkPetscError(ierr, "setupPetsc1DMonitor: TSMonitorSet (computeFluence) failed.");
...
// computeXenonRetention1D will be called at each timestep
   ...
   checkPetscError(ierr, "setupPetsc1DMonitor: TSMonitorSet (computeXenonRetention1D) failed.");
