...
// Create the desired type of handler registry.
 bool 
 initializeDimension(xolotlCore::Options& options) 
 {
  bool ret = true;
...
// Provide access to our handler registry.
 std::shared_ptr<xolotlSolver::ISolverHandler> 
 getSolverHandler() 
 {
  if ( !theSolverHandler )
  {
// We have not yet been initialized.
   throw std::string("\nxolotlFactory: solver requested but it has not been initialized.");
  }
