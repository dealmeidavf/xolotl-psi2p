...
// Create the desired type of handler registry.
 bool 
 initializeTempHandler( xolotlCore::Options &options ) 
 {
// Get the current process ID
  ...
  if (options.useConstTemperatureHandlers() && options.useTemperatureProfileHandlers())
  {
...
// Only print the error message once when running in parallel
   if (procId == 0) 
   {
    std::cerr << "\nWarning: Temperature information has not been given. Defaulting to constant temperature = 1000K " << std::endl;
   }
   auto temp = options.getConstTemperature();
// we are to use a constant temperature handler
   theTemperatureHandler = std::make_shared<xolotlCore::TemperatureHandler>(temp);
...
// Provide access to our handler registry.
 std::shared_ptr<xolotlCore::ITemperatureHandler> 
 getTemperatureHandler() 
 {
  if ( !theTemperatureHandler )
  {
// We have not yet been initialized.
   throw std::string("\nxolotlFactory temperature handler requested but it has not been initialized.");
  }
