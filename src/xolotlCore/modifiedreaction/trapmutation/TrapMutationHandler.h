...
//! The constructor
     Desorption( int s, double p ) : size(s), portion(p) {}
   }; // end of class Desorption
...
/**
 * This method fills two vectors to define the modified trap-mutation: for the first 
 * one, the first value corresponds to the depth at which the He1 cluster undergo 
 * trap-mutation (if the value is negative it means that it doesn't TM), the second 
 * value correspond to He2, etc.; the second vector gives the size of the vacancies 
 * into which He trap-mutates. Information about desorption is also initialized here.
 ...
 * The constructor
 */
   TrapMutationHandler() : kMutation(0.0), kDis(1.0), attenuation(true), 
                          desorp(0, 0.0) {}
 ...
 */
   void setAttenuation( bool isAttenuationh );
